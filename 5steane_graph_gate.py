#Graph producing program for [5,1,3] Steane QEC code with depolarizing noise model on all gates
#Probability of depolarizing gate error is sweeped between 10^(-4) and 0.063 in logscale and resulting P_success of the code is plotted
...
points = 50
p_g = np.logspace(-4,-1.2,points)
pteo_32u = []
pfail_teo = []

zoomb = 10**(-3) #zoom boundary
posiz = 0
...
plt.xticks([10**(-4), 10**(-3), 10**(-2), 10**(-1.2)])
plt.xlim([1e-04, 10**(-1.2)])
#plt.ylim([0.7,1])
plt.savefig("cyc5steane_Psucc_vs_Pgate.png", dpi=1000)
plt.show()
...
plt.xticks([10**(-4), 10**(-3)])
plt.xlim([1e-04, zoomb])
plt.savefig("cyc5steane_Pfail_vs_Pgate.png", dpi=1000)
plt.show()