...
nerr = int((n-1)/2)
err = [[0]*n for i in range(nerr)]    #err[nerr][nqubit]

for i in range (nerr):
    err[i][0:(i+1)]=[1]*(i+1)

dispo = [[] for x in range(n)]

for i in range(nerr):
    for x in set(permutations(err[i])):
        for j in range (n):
            if(list(x)[j]==1):
                dispo[j].append(list(x))

ndispo=len(dispo[0])
table = [[0]*ndispo for x in range(n)]

for i in range (n):
    for j in range (ndispo):
        tempstr = ""
        for k in range (n-1,0,-1):
            if dispo[i][j][k]==dispo[i][j][k-1]:
                tempstr = tempstr + '0'
            else:
                tempstr = tempstr + '1'
...
for i in range (rs):
    ...
    for qbit in range(n):
        for disp in range(ndispo):
            qc.x(data[qbit]).c_if(sbit, table[qbit][disp])
